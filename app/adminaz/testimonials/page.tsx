      ...
      <div className="min-h-screen -m-4 md:-m-12 bg-white">
        ...
        <div className="max-w-5xl mx-auto p-6 md:p-20 grid grid-cols-1 lg:grid-cols-3 gap-12 md:gap-20">
          <aside className="space-y-8">
            <div>
              <label className="text-[10px] font-black uppercase tracking-widest text-apple-text-secondary mb-4 block text-center">User Avatar</label>
              <div 
                onClick={() => setActiveMediaPicker("avatar")}
                className="w-32 h-32 md:w-48 md:h-48 mx-auto bg-[#f5f5f7] rounded-full border-2 border-dashed border-apple-border flex flex-col items-center justify-center gap-2 cursor-pointer hover:bg-gray-50 overflow-hidden relative group"
              >
                {formData.avatar_url ? (
                  <>
                    ...
                    <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                      <span className="bg-white/90 backdrop-blur px-3 py-1.5 rounded-full text-[10px] font-bold shadow-xl">Change</span>
                    </div>
                  </>
                ) : (
            ...
            <div className="pt-8">
              <label className="text-[10px] font-black uppercase tracking-widest text-apple-text-secondary mb-4 block">Rating</label>
          ...
          <div className="lg:col-span-2 space-y-12">
            <section className="space-y-8">
              <h2 className="text-sm font-black uppercase tracking-widest text-apple-text-secondary">Client Details</h2>
              
              <div className="space-y-3">
                <label className="text-xs font-bold text-apple-text px-1">Full Name</label>
                <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full p-5 bg-[#f5f5f7] rounded-3xl border-none focus:ring-2 focus:ring-apple-accent text-lg font-bold" placeholder="e.g. Sarah Chen" />
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  <label className="text-xs font-bold text-apple-text px-1">Role/Company (EN)</label>
                  <input type="text" value={formData.role_en} onChange={(e) => setFormData({ ...formData, role_en: e.target.value })} className="w-full p-4 bg-[#f5f5f7] rounded-2xl border-none focus:ring-2 focus:ring-apple-accent font-medium" placeholder="e.g. CTO at Nexus AI" />
                </div>
                <div className="space-y-3">
                  <label className="text-xs font-bold text-apple-text px-1">Vị trí (VI)</label>
                  <input type="text" value={formData.role_vi} onChange={(e) => setFormData({ ...formData, role_vi: e.target.value })} className="w-full p-4 bg-[#f5f5f7] rounded-2xl border-none focus:ring-2 focus:ring-apple-accent font-medium" placeholder="VD: CTO tại Nexus AI" />
                </div>
              </div>

              <div className="space-y-8 pt-4">
                <div className="space-y-3">
                  <label className="text-xs font-bold text-apple-text px-1 flex items-center gap-2">
                    <Quote className="w-3 h-3" /> Testimonial Content (EN)
                  </label>
                  <textarea rows={4} value={formData.content_en} onChange={(e) => setFormData({ ...formData, content_en: e.target.value })} className="w-full p-5 bg-[#f5f5f7] rounded-3xl border-none focus:ring-2 focus:ring-apple-accent resize-none leading-relaxed" placeholder="What the client said in English..." />
                </div>
                <div className="space-y-3">
                  <label className="text-xs font-bold text-apple-text px-1 flex items-center gap-2">
                    <Quote className="w-3 h-3" /> Nội dung đánh giá (VI)
                  </label>
                  <textarea rows={4} value={formData.content_vi} onChange={(e) => setFormData({ ...formData, content_vi: e.target.value })} className="w-full p-5 bg-[#f5f5f7] rounded-3xl border-none focus:ring-2 focus:ring-apple-accent resize-none leading-relaxed" placeholder="Khách hàng nói gì bằng tiếng Việt..." />
                </div>
              </div>
            </section>
          </div>
  ...
  return (
    <div className="space-y-8 md:space-y-12">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          <h1 className="text-3xl md:text-5xl font-bold tracking-tight text-apple-text">Testimonials</h1>
          <p className="text-apple-text-secondary mt-2 text-base md:text-lg">What clients say about AZLABS precision.</p>
        </div>
      ...
      {loading ? (
        <div className="py-40 flex flex-col items-center gap-4">
          ...
          <p className="text-apple-text-secondary font-medium animate-pulse">Fetching client feedback...</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {testimonials.map((t) => (
            <div 
              ...
              className="group bg-white p-6 md:p-10 rounded-[40px] border border-apple-border flex flex-col gap-6 hover:shadow-2xl transition-all cursor-pointer hover:border-black/5"
            >
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-4">
                  <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-apple-border group-hover:scale-105 transition-transform duration-500">
                    {t.avatar_url ? (
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-apple-text">{t.name}</h3>
                    <p className="text-apple-text-secondary text-xs font-medium uppercase tracking-widest mt-1">{t.role_en}</p>
                  </div>
                </div>
              ...
              <div className="relative">
                <Quote className="absolute -left-2 -top-2 w-8 h-8 text-apple-accent/5 -z-10" />
                <p className="text-apple-text-secondary font-medium italic line-clamp-3 leading-relaxed">
                  "{t.content_en}"
                </p>
              </div>
